...
/* Add custom intro section [Extend for more options in ESTSB update] */
...
//Output header above slider - ESTSB specific
...
//Output header below slider - ESTSB specific
...
/* ----------------------------------------------------------------------------------
	BACK UP OPTIONS TO PAGE "ESTSB CREATED CONTENT BACKUP"
---------------------------------------------------------------------------------- */
...
global $wp_customize;
	...
	if ( $thinkup_general_backupswitch == '1' ) {
		...
		$output_header   = '';
		$output_header  .= '<-----------------------------------------------------------' . "\n";
		$output_header  .= esc_html__( 'ESTSB Created Content Backup', 'ESTSB' ) . "\n";
		$output_header  .= esc_html__( 'This page contains a backup of content created by the ESTSB WordPress Theme. ', 'ESTSB' );
		$output_header  .= esc_html__( 'The purpose for the backup is to prevent content loss on theme switch.', 'ESTSB' );
		$output_header  .= esc_html__( 'When a user switches themes this content will still be available to the user when setting up their site on the new theme.', 'ESTSB' ) . "\n";
		$output_header  .= esc_html__( 'Please note the following : ', 'ESTSB' ) . "\n";
		$output_header  .= ' * ' . esc_html__( 'Leave this page as private, available only to users with admin privledges.', 'ESTSB' ) . "\n";
		$output_header  .= ' * ' . esc_html__( 'You can delete this page any time and regenerate it from within the ESTSB options menu, General section.', 'ESTSB' ) . "\n";
		$output_header  .= '----------------------------------------------------------->' . "\n";
		$output_header  .= "\n" . "\n";

		// Backup page setup.
		$postarray['post_title']     = 'ESTSB Created Content Backup'; // translate ok.
		$postarray['post_type']      = 'page';
		$postarray['post_status']    = 'private';
		$postarray['comment_status'] = 'closed';
		$page                        = get_page_by_title( 'ESTSB Created Content Backup' );
