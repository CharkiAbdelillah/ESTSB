...
function thinkup_title_select() {
	...
	if ( is_page() ) {
		...
		printf( '<span>' . esc_html__( 'Blog Post Image: ', 'ESTSB' ) . '</span>' . '%s', esc_html( get_the_title( $post->post_parent ) ) );
	} else if ( is_single() ) {
		printf( '<span>%s</span>', html_entity_decode( esc_html( get_the_title() ) ) );
	} else if ( is_search() ) {
		printf( '<span>' . esc_html__( 'Search Results: ', 'ESTSB' ) . '</span>' . '%s', esc_html( get_search_query() ) );
	} else if ( is_404() ) {
		printf( '<span>' . esc_html__( 'Page Not Found', 'ESTSB' ) . '</span>' );
	} else if ( is_category() ) {
		printf( '<span>' . esc_html__( 'Category Archives: ', 'ESTSB' ) . '</span>' . '%s', single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		printf( '<span>' . esc_html__( 'Tag Archives: ', 'ESTSB' ) . '</span>' . '%s', single_tag_title( '', false ) );
	} elseif ( is_author() ) {
		the_post();
		printf( '<span>' . esc_html__( 'Author Archives: ', 'ESTSB' ) . '</span>' . '%s', get_the_author() );
		rewind_posts();
	} elseif ( is_day() ) {
		printf( '<span>' . esc_html__( 'Daily Archives: ', 'ESTSB' ) . '</span>' . '%s', get_the_date() );
	} elseif ( is_month() ) {
		printf( '<span>' . esc_html__( 'Monthly Archives: ', 'ESTSB' ) . '</span>' . '%s', get_the_date( 'F Y' ) );
	} elseif ( is_year() ) {
		printf( '<span>' . esc_html__( 'Yearly Archives: ', 'ESTSB' ) . '</span>' . '%s', get_the_date( 'Y' ) );
	} elseif ( is_archive() ) {
		printf( get_the_archive_title() );
	} elseif ( is_tax() ) {
		printf( esc_html( get_queried_object()->name ) );
	} elseif ( thinkup_check_isblog() ) {
		printf( '<span>' . esc_html__( 'Blog', 'ESTSB' ) . '</span>' );
	} else {
		printf( '<span>%s</span>', html_entity_decode( esc_html( get_the_title() ) ) );
	}
}
...
$thinkup_general_breadcrumbdelimeter = thinkup_var ( 'thinkup_general_breadcrumbdelimeter' );
	...
	if ( ! is_front_page() ) {
		...
		if ( is_single() ) {
			...
			$output .= '<span class="breadcrumbs-cat">' . __( 'Archive Category: ', 'ESTSB' ) . '</span>' . esc_html( single_cat_title("", false) );
		} elseif ( is_tag() ) {
			$output .= '<span class="breadcrumbs-tag">' . __( 'Posts Tagged: ', 'ESTSB' ) . '</span>' . single_tag_title("", false);
		} elseif ( is_day()) {
			...
			$output .= esc_html__( 'Search Results for: ', 'ESTSB' ) . esc_html( get_search_query() ) . '"';
			...
			$output .= esc_html__( 'Archived Article(s) by Author: ', 'ESTSB' ) . esc_html( $user_info->display_name );
		} elseif ( is_404() ) {
			$output .= esc_html__( 'Error 404 - Not Found.', 'ESTSB' );
		} elseif ( is_archive() ) {
			$output .= get_the_archive_title();
		} elseif( is_tax() ) {
			$output .= esc_html( get_queried_object()->name );
		} elseif ( thinkup_check_isblog() ) {
			$output .= esc_html__( 'Blog', 'ESTSB' );
		} else {
			$output .= html_entity_decode( esc_html( get_the_title() ) );
		}
...
if ( ! function_exists( 'thinkup_input_showimagesizes' ) ) { 
	function thinkup_input_showimagesizes($sizes) {
		...
		// 1 Column Layout
		$sizes['column1-1/2'] = __( 'Full - 1:2', 'ESTSB' );
		$sizes['column1-1/3'] = __( 'Full - 1:3', 'ESTSB' );
		$sizes['column1-1/4'] = __( 'Full - 1:4', 'ESTSB' );
		$sizes['column1-2/5'] = __( 'Full - 2:5', 'ESTSB' );

		// 2 Column Layout
		$sizes['column2-1/1'] = __( 'Half - 1:1', 'ESTSB' );
		$sizes['column2-1/2'] = __( 'Half - 1:2', 'ESTSB' );
		$sizes['column2-2/3'] = __( 'Half - 2:3', 'ESTSB' );
		$sizes['column2-3/5'] = __( 'Half - 3:5', 'ESTSB' );

		// 3 Column Layout
		$sizes['column3-1/1'] = __( 'Third - 1:1', 'ESTSB' );
		$sizes['column3-2/5'] = __( 'Third - 2:5', 'ESTSB' );
		$sizes['column3-2/3'] = __( 'Third - 2:3', 'ESTSB' );
		$sizes['column3-3/4'] = __( 'Third - 3:4', 'ESTSB' );

		// 4 Column Layout
		$sizes['column4-1/1'] = __( 'Quarter - 1:1', 'ESTSB' );
		$sizes['column4-2/3'] = __( 'Quarter - 2:3', 'ESTSB' );
		$sizes['column4-3/4'] = __( 'Quarter - 3:4', 'ESTSB' );
...
function thinkup_googlefonts_url() {
    ...
    $font_translate = _x( 'on', 'Lato font: on or off', 'ESTSB' );
